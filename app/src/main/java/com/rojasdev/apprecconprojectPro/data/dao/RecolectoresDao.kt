...
import com.rojasdev.apprecconprojectPro.data.dataModel.idStateFirebaseCollectorAndCollection
import com.rojasdev.apprecconprojectPro.data.dataModel.monthPdf
import com.rojasdev.apprecconprojectPro.data.dataModel.totalMonthPdf
import com.rojasdev.apprecconprojectPro.data.dataModel.weekPdf
...
interface RecolectoresDao {
    ...
    @Insert
    suspend fun addM(recolector : List<RecolectoresEntity>)

    @Query("UPDATE Recolectores SET  estado_recolector = 'archive' WHERE PK_ID_Recolector = :id")
    suspend fun updateCollectorState(id:Int)

    @Query("UPDATE Recolectores SET  estado_firebase = :state WHERE PK_ID_Recolector = :id")
    suspend fun updateCollectorStateFirebase(id:Int, state: Boolean)
    ...
    @Query("SELECT * FROM recolectores WHERE estado_firebase == :state")
    suspend fun getAllRecolectorFirebase(state: Boolean): List<RecolectoresEntity>
    ...
    @Query("SELECT PK_ID_Recolector " +
            "FROM recolectores " +
            "WHERE estado_firebase Like false "
    )
    suspend fun getCollectorStateFirebase(): List<Long>
    ...
    @Query("SELECT * FROM recolectores WHERE estado_firebase == 0")
    suspend fun getCollectorNoUpdate(): List<RecolectoresEntity>
